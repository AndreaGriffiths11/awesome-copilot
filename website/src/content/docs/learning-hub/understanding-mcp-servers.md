...
- **Microsoft Entra ID (Azure AD)**: MCP servers that authenticate via Microsoft Entra ID are fully supported. Once you complete the initial login, the CLI caches the authentication and **will not show the consent screen on subsequent connections** — you authenticate once per session rather than every time the server reconnects. On **Windows (v1.0.81+)**, Entra ID authentication uses the OS authentication broker (WAM), which typically signs you in silently using your existing Windows credentials — no browser prompt required. Other platforms and machines without the broker library keep the existing browser flow.
