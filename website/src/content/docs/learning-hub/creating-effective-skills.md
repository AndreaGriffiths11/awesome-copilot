...
Alternatively, use the `copilot plugins install --skill` command *(v1.0.72+)*, which follows the plugin installation flow and supports an optional `--scope project` flag to install into the repository:

```bash
copilot plugins install --skill ./my-skill/
copilot plugins install --skill https://example.com/skill.zip --scope project
```
