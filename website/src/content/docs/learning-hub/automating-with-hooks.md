...
> **Note (v1.0.72+)**: An `agentStop` hook that _always_ blocks no longer loops indefinitely. After 8 consecutive blocks, the CLI forces continuation and passes `stop_hook_active: true` in the hook's JSON input so your script can detect this condition and self-limit. Use this flag to gracefully exit when the forced-continuation threshold is reached:
>
> ```bash
> #!/usr/bin/env bash
...
>   # CLI is forcing continuation — exit cleanly to avoid an infinite loop
>   exit 0
> fi
>
