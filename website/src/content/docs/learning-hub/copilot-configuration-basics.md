...
You can also load skills and custom agents from additional directories at startup using the `--add-dir` flag (v1.0.81+). Any skills or agent files found in the specified directory are loaded as if they were in a standard discovery path:

```bash
copilot --add-dir ~/my-shared-agents
```

This is useful when you have a shared folder of agents or skills that you want to use across multiple projects without copying them into each repository.
...
| `defaultPermissionMode` | Default approval behavior for new sessions (`auto`, `manual`, or `allow-all`) (v1.0.81+) |
