...
| `defaultMode` | Default startup mode for new interactive sessions: `interactive` (default) or `autopilot`. Use `autopilot` if you usually want to skip per-turn confirmation prompts (v1.0.81+) |
| `defaultPermissionMode` | Default approval behavior for new interactive sessions. Controls whether the CLI requires explicit approval for tool use. Accepts the same values as the `/autopilot` and `/allow-all` commands (v1.0.81+) |
