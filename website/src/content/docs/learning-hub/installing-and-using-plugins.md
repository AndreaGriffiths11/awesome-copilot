...
**Auto-updating a marketplace** (v1.0.79+): Add `"autoUpdate": true` to an `extraKnownMarketplaces` entry to automatically update all plugins from that marketplace at session start. This is useful for shared team marketplaces where you always want the latest plugin versions:
