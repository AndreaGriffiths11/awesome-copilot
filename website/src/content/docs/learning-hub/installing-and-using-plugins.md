...
You can enable or disable individual components of installed plugins — including plugins themselves, agents, LSP servers, and hooks — without uninstalling them (v1.0.76+). Open `/plugins` in an interactive session to see the controls, or use flags:
...
# Enable/disable a specific agent from a plugin
/plugins enable --plugin my-plugin agent-name

# Enable/disable a hook from a plugin
/plugins disable --plugin my-plugin hook-name

# Enable/disable an MCP server
/plugins enable --mcp my-mcp-server
```

This is useful for temporarily turning off a component that's causing slowdowns or conflicts without losing your installation.

### Open Plugin Spec v1

Plugins support the **Open Plugin Spec v1** manifest format (v1.0.74+), which enables broader compatibility with tools that use the open plugin ecosystem — including `mcp.json` configuration. If you're authoring a plugin intended for use beyond GitHub Copilot CLI, you can use an Open Plugin Spec v1 `plugin.json` and it will be recognized automatically.
