...
With `autoUpdate` enabled, the CLI checks for plugin updates from that marketplace each time a session starts, so your team always has the latest versions without manually running `copilot plugin update`.
