...
Yes. As of v1.0.72, **multi-turn subagents are always enabled** — you can send follow-up messages to a running subagent without any special configuration. Earlier releases required opting in to this behavior; it is now the default for all sessions.

**How do I trace which agent sent a prompt in a multi-agent session?**

Subagent timelines (v1.0.74+) now clearly identify whether each prompt was sent by the main agent or by another subagent, making it much easier to follow the flow of orchestrated work. Open the `/tasks` panel to view the full timeline for a session, including the source of each prompt and response in the correct order.
