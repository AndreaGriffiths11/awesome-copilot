...
> **v1.0.71+**: The default maximum sub-agent nesting depth was **lowered from 6 to 4** to curb runaway recursive delegation. Usage-based billing users can still adjust `subagents.maxDepth` up to 128 via `/settings`, but the tighter default protects most users from unbounded cost and context consumption.

**Can I send follow-up messages to a running subagent?**

Yes (v1.0.72+). Multi-turn subagents are **always enabled** — you can send follow-up messages to a running agent at any time, not just after it finishes. This makes it easier to steer or refine a long-running subagent task mid-flight without cancelling it.
