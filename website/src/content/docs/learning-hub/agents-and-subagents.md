...
> **Changed in v1.0.71**: The default maximum sub-agent nesting depth was lowered from 6 to 4 to prevent runaway recursive delegation. If you use usage-based billing and need deeper chains, you can raise `subagents.maxDepth` (up to 128) in `/settings`.

**Can I send follow-up messages to a subagent that's already running?**

Yes. As of v1.0.72, **multi-turn subagents are always enabled**, so you can send additional messages to agents while they are actively working. This is especially useful for redirecting or providing clarification mid-task without waiting for the agent to finish.
